...
extension Waveform1D where T: BinaryFloatingPoint {
    ...
    public func integrate(initialValue: T = T.zero) -> Waveform1D<T> {
        ...
        let dtValue = T(dt.secondsAsDouble) / 2

        for i in 1..<values.count {
            let trapezoidArea = (values[i - 1] + values[i]) * dtValue
    ...
    public func derivative() -> Waveform1D<T> {
        ...
        var derivative: [T] = []
        let dtValue = T(dt.secondsAsDouble) * 2.0

        // Forward difference for first point
        derivative.append((values[1] - values[0]) / T(dt.secondsAsDouble))

        // Central difference for middle points
        for i in 1..<(values.count - 1) {
            derivative.append((values[i + 1] - values[i - 1]) / dtValue)
        }

        // Backward difference for last point
        if values.count > 1 {
            derivative.append((values[values.count - 1] - values[values.count - 2]) / T(dt.secondsAsDouble))
        }
