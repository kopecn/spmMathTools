...
extension Waveform1D where T: BinaryFloatingPoint & Comparable {
    ...
    public func upperLowerEnvelopes(
        windowSize: Int = 5,
        ...
        -> (upper: Waveform1D<T>, lower: Waveform1D<T>)
    {
        ...
        guard values.count > windowSize else {
            let constantUpper = Waveform1D(
                values: Array(repeating: maximum ?? T.zero, count: values.count),
                dt: dt,
                t0: t0
            )
            let constantLower = Waveform1D(
                values: Array(repeating: minimum ?? T.zero, count: values.count),
                dt: dt,
                t0: t0
            )
    ...
    private func interpolateEnvelope(
        points: [(index: Int, value: T)],
